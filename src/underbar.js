  ...
  _.filter = function(collection, test) {
    var newArr = [];
     _.each(collection, function(value) {
          if (test(value)) {newArr.push(value)}
     })
    return newArr
  };
...
//result for object 1, and 3 is displayed
  ...
  _.map = function(collection, iterator) {
    ...
    // the members, it also maintains an array of results.
     var newArr = [];
    _.each(collection, function(value) {newArr.push(iterator(value))});
    return newArr;
  };
 ...
 _.every = function(collection, iterator) {
    iterator = iterator || _.identity;   ////if no iterator passed, then use _.identity
    return _.reduce(collection, function(isTrue, value) { 
         if(isTrue) {
        return !!iterator(value) //return true
       } else {return !!false;}
    }, true);
};
  ...
  //also can be done using a simple for loop
_.some = function(collection, iterator) {
  ...
  var temp, randomLoc;
  while(--i > 0)  {
    randomLoc = Math.floor(Math.random() * (i+1));
    temp = newArr[randomLoc];
    newArr[randomLoc] = newArr[i];
    newArr[i] = temp;
  }
  return newArr;
  };
